...
def prepare(rType="MAIN"):
    ...
    printc("Preparazione Installazione")
    for rFile in ["/var/lib/dpkg/lock-frontend", "/var/cache/apt/archives/lock", "/var/lib/dpkg/lock"]:
        try: os.remove(rFile)
        except: pass
    os.system("apt-get update > /dev/null")
    printc("Rimozione libcurl4 se installato")
    os.system("apt-get remove --auto-remove libcurl4 -y > /dev/null")
    for rPackage in rPackages:
        printc("Installazione %s" % rPackage)
        os.system("apt-get install %s -y > /dev/null" % rPackage)
    printc("Installazione libpng")
    ...
    try:
        ...
        # Create User
        printc("Creazione utente xtreamcodes")
...
def install(rType="MAIN"):
    ...
    if os.path.exists("/tmp/xtreamcodes.tar.gz"):
        printc("Installazione del Software")
...
def mysql(rUsername, rPassword):
    ...
    printc("Inserisci la password di ROOT per MySql Server:", col.WARNING)
    for i in range(5):
        rMySQLRoot = raw_input("  ")
        print " "
        if len(rMySQLRoot) > 0: rExtra = " -p%s" % rMySQLRoot
        else: rExtra = ""
        printc("Cancellare il database attuale e ricrearlo? y/n", col.WARNING)
...
def configure():
    printc("Configurando il Sistema")
...
def start(first=True):
    if first: printc("Avviando Xtream Codes")
...
if __name__ == "__main__":
    ...
    rType = raw_input("  Installation Type [MainServer, LoadBalancer, Admin]: ")
    print " "
    if rType.upper() in ["MAIN", "LB"]:
        ...
        if len(rHost) > 0 and len(rPassword) > 0 and rServerID > -1:
            printc("Avviare Installazione del Pannello? y/n", col.WARNING)
